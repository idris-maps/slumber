...
export default (db: Db, closeDb?: boolean) =>
  async (collection: string, __id: string, key: string, data: any): Promise<HandlerResponse> => {
    ...
    const patchItem = req.items.patch(db)
    ...
    if (!allowedMethods.includes('patch')) { return respond({ status: 403 }) }

    const check = validate(omit(['required'], col.schema), data)
    if (!check.valid && check.error) { return respond({ status: 400, data: { error: check.error } }) }

    const item = await patchItem(col.id, __id, data)
    return item
